...
export class ArrangementEngine {
  public selectArrangement(availableArrangements: Arrangement[], ctx: GeneratorContext): Arrangement {
    // Ensure arrangements dynamically randomize based on the deterministic seed
    // preventing the engine from just picking the first one repetitively.
    if (availableArrangements.length > 0) {
      const index = Math.floor(ctx.rng.float() * availableArrangements.length);
      return availableArrangements[index];
    }
    return defaultArrangement;
  }

  public generatePhrases(phrasePool: Phrase[], ctx: GeneratorContext): Phrase | null {
    // Actually utilize the varied phrase pool to create distinct musical decorations
    if (phrasePool.length > 0) {
      const index = Math.floor(ctx.rng.float() * phrasePool.length);
