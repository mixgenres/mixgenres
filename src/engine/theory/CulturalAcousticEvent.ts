...
export class CulturalAcousticEventNode {
  public attack: number = 0.05;
  public decay: number = 0.1;
  public sustain: number = 0.5;
  public release: number = 0.2;
  public isResonant: boolean = false;
  public bodyResonance: number = 0.0;
  public strikeForce: number = 0.5;
  public articulation?: string;
  public duration: number = 0.5;
  public velocity: number = 70;

  public processAcoustics(metadata: EventMetadata = {}) {
    // Distinct acoustic envelopes to support varied decorations, long notes, and resonant strikes
    if (this.articulation === 'staccato' || this.articulation === 'strike') {
      this.attack = 0.02;
      this.decay = 0.15;
      this.sustain = 0.1;
      this.release = 0.15;
    } else {
      // Preserve resonance and let long notes ring out naturally
      this.attack = metadata.customAttack || 0.05;
      this.decay = metadata.customDecay || 0.2;
      this.sustain = metadata.sustainLevel || 0.85;
      this.release = metadata.resonanceTime || this.duration + 1.5;
    }

    // Data is additive: map sophisticated metadata directly to DSP parameters
    this.isResonant = metadata.isResonant || false;
    this.bodyResonance = this.isResonant ? (metadata.resonanceDepth || 1.2) : 0.0;
    this.strikeForce = metadata.strikeForce || (this.velocity / 127.0);
  }
}

export function processAcoustics(event: CulturalAcousticEvent, metadata: EventMetadata = {}): CulturalAcousticEvent {
  const duration = event.duration ?? 0.5;
  const velocity = event.velocity ?? 70;
  const art = event.articulation || event.techniqueModifier || event.action?.technique || event.action?.type;

  if (art === 'staccato' || art === 'strike') {
    event.attack = 0.02;
    event.decay = 0.15;
    event.sustain = 0.1;
    event.release = 0.15;
  } else {
    event.attack = metadata.customAttack || 0.05;
    event.decay = metadata.customDecay || 0.2;
    event.sustain = metadata.sustainLevel || 0.85;
    event.release = metadata.resonanceTime || duration + 1.5;
  }

  event.isResonant = metadata.isResonant || false;
  event.bodyResonance = event.isResonant ? (metadata.resonanceDepth || 1.2) : 0.0;
  event.strikeForce = metadata.strikeForce || (velocity / 127.0);
  return event;
}
