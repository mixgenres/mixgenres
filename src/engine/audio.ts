...
import { resolveDialect } from './dialects';
...
export function createSink(): TransportSink {
  return {
    ...
    noteOn(trackId, midi, vel, time, articulation, frequencyHz) {
      ...
      if (articulation) {
        const artLow = articulation.toLowerCase();
        if (artLow.includes('arco') || artLow.includes('bowed')) {
          ...
          actionType = 'arrastre';
        } else if (artLow.includes('slap') && !artLow.includes('bass')) {
          actionType = 'slap';
      ...
      bandWorklet.postEvent({
        ...
        action: { type: actionType as any, force: vel01, contactPoint, mass, technique: articulation },
