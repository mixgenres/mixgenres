...
export const SOUNDFONT_MANIFEST: SoundfontManifestEntry[] = [
  { id: 'main', file: 'GeneralUser-GS.sf2', bankOffset: 0, primary: true },
  { id: 'celtix', file: 'ethnic/celtix.sf2', bankOffset: 0 },
  { id: 'bagpipes', file: 'ethnic/bagpipes.sf2', bankOffset: 1 },
  { id: 'erhu', file: 'ethnic/erhu.sf2', bankOffset: 3 },
  { id: 'shakuhachi', file: 'ethnic/shakuhachi.sf2', bankOffset: 4 },
  { id: 'guzheng', file: 'ethnic/guzheng.sf2', bankOffset: 5 },
  { id: 'pipa', file: 'ethnic/pipa.sf2', bankOffset: 7 },
];

export const INSTRUMENT_CATALOG: InstrumentDef[] = [
  // — bellows & keys —
  { id: 'bandoneon', name: 'Bandoneon', family: 'bellows-and-keys', program: 23, voicing: 'chord', note: 'Breathy, can bite or sigh' },
  ...
  // — plucked —
  { id: 'guitar', name: 'Nylon guitar', family: 'plucked', program: 24, voicing: 'chord' },
  ...
  { id: 'uilleann-pipes', name: 'Uilleann pipes', family: 'winds', program: 109, voicing: 'single', soundfont: { soundfontId: 'celtix', bankMSB: 0, bankLSB: 0, program: 109 }, note: 'Celtix bagpipe-family preset as a timbral proxy; Irish phrase rules remain distinct' },
...
export function effectiveSoundfontPreset(instrumentId: string): { soundfontId: string; bankMSB: number; bankLSB: number; program: number } | null {
  const ref = INSTRUMENTS_BY_ID[instrumentId]?.soundfont;
  if (!ref) return null;
  const manifest = SOUNDFONT_MANIFEST.find(f => f.id === ref.soundfontId);
  return {
    soundfontId: ref.soundfontId,
    bankMSB: Math.max(0, Math.min(127, (ref.bankMSB ?? 0) + (manifest?.bankOffset ?? 0))),
